...
namespace ORB_SLAM2
{
  ...
  MapPoint::MapPoint(const cv::Mat &Pos, KeyFrame *pRefKF, Map *pMap)
      : mnFirstKFid(pRefKF->mnId), mnFirstFrame(pRefKF->mnFrameId), nObs(0),
        ...
        mfMaxDistance(0), mpMap(pMap), RealPositionKnown(false), assigned(false)
  {
    if (!Pos.empty())
  ...
  MapPoint::MapPoint(const cv::Mat &Pos, Map *pMap, Frame *pFrame,
      ...
      : mnFirstKFid(-1), mnFirstFrame(pFrame->mnId), nObs(0),
        ...
        mbBad(false), mpReplaced(NULL), mpMap(pMap), RealPositionKnown(false),
        assigned(false)
  {
    Pos.copyTo(mWorldPos);
  ...
  void MapPoint::ComputeDistinctiveDescriptors()
  {
    ...
    std::vector<std::vector<int>> Distances(N, std::vector<int>(N, 0.0));
    ...
    for (size_t i = 0; i < N; i++)
    {
      vector<int> vDists (Distances[i].begin(), Distances[i].end());  // not sure that is correct
