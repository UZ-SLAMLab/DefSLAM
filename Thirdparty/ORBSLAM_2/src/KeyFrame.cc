...
namespace ORB_SLAM2
{
  ...
  KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB)
      : mnFrameId(F.mnId), mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS),
        ...
        mvpMapPoints(F.mvpMapPoints), mvpMapPointsCorr(F.mvpMapPointsCorr),
  ...
  vector<MapPoint *> KeyFrame::GetMapPointMatchesCorr()
  {
    unique_lock<mutex> lock(mMutexFeatures);
    return mvpMapPoints;
  }
  ...
  cv::Mat KeyFrame::getIm() { return RGBimage.clone(); }
  ...
  void KeyFrame::SaveKeyframe(string a) { cv::imwrite(a, KFimage); }

  void KeyFrame::SavePose(string a)
  {
    cv::FileStorage file(a, cv::FileStorage::WRITE);
    file << "Tcw" << Tcw;
  }

  void KeyFrame::SavePoints(string a)
  {
    ofstream myfile;
    myfile.open(a);
    int i(0);
    for (std::vector<MapPoint *>::iterator itMP = mvpMapPoints.begin();
         itMP != mvpMapPoints.end(); itMP++)
    {
      MapPoint *pMP = *itMP;
      if (pMP)
        myfile << to_string(pMP->mnId) << " " << to_string(mvKeysUn[i].pt.x)
               << " " << to_string(mvKeysUn[i].pt.y) << std::endl;
      i++;
    }
  }
  cv::KeyPoint KeyFrame::ProjectPoints(const cv::Mat &P)
  {
    ...
    if (PcZ < 0.0f)
      return cv::KeyPoint(cv::Point2f(-1, -1), 10);
